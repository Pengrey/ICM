...
class RegisterPage : AppCompatActivity() {
    ...
    private fun register(nameField: EditText, emailField: EditText, passwordField: EditText, cidField: EditText, prefs: SharedPreferences) {
        ...
        if (email.isNotEmpty() && cid.isNotEmpty() && password.isNotEmpty() && name.isNotEmpty()){
            // Create Rider
            val rider: Rider = Rider(email,password,name, cid)
            ...
            val request = Request.Builder()
                .url("http://51.142.110.251/api/v1/accounts/register/rider")
                .post(body)
                .build()
